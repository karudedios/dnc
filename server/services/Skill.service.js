  ...
  static single(query) {
    return Querier
      .append({ as: 'model', from: boundPromisedCallback('findOne', query), where: [
        [() => new HttpStatusModel(404, "Skill not found"), isTruthy]
      ]})
      .select(({ model }) => model)
  }
  ...
  static where(query) {
    return Querier
      .append({ as: 'models', from: boundPromisedCallback('find', query), where: [
        [() => new HttpStatusModel(404, "Skills not found"), models => models.length > 0]
      ]})
  ...
  static save(model) {
    return Querier
      .append({ as: 'model', from: promisedCallback(new Model(model), 'save'), where: [
        [() => new HttpStatusModel(500, "Could not create the Skill"), isTruthy]
      ]})
      .select(({ model }) => model)
  }
  ...
  static update(_id, updated) {
    return Querier
      .append({ as: 'model', from: boundPromisedCallback('findByIdAndUpdate', _id, updated, { multi: true }), where: [
        [() => new HttpStatusModel(404, "No Skill found with the ID specified"), isTruthy]
      ]})
      .select(({ model }) => model)
  }
  ...
  static delete(_id) {
    return Querier
      .append({ as: 'model', from: boundPromisedCallback('findByIdAndRemove', _id), where: [
        [() => new HttpStatusModel(404, "Not Skill found with the ID specified"), isTruthy]
      ]})
      .select(({ model }) => model)
  }
};
